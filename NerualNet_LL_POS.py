...
def collate_batch(batch):
    ...
    for _label, _text in batch:
        label_list.append(batch['log_level'])
...
def PosNum(txt):
    ...
    partOfSpeech = []
    for token in doc:
        if len(partOfSpeech) >= max_size:
            break
        partOfSpeech.append(np.int64(token.pos))

    while len(partOfSpeech < max_size):
        partOfSpeech.append(0)
...
df = preprocess(df)
print(df['posNum'])
df = df.drop(['project', 'static_text'], axis=1)
column_names = ['log_level', 'posNum']
df['log_level'] = df['log_level'].replace('info', 0.0).replace('error', 1.0).replace('warn', 2.0)
