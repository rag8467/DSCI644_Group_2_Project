...
#Referenced https://pytorch.org/tutorials/beginner/text_sentiment_ngrams_tutorial.html
...
'''
def predict_expected(dataloader):
    model.eval()
    total_acc, total_count = 0, 0
...
'''
Model for training text classification
'''
...
'''
Concatenates an entire batch into a single tensor to send to the embedding layer.
'''
...
'''
Feeds the input text to the text pipeline which is then feed to the model to predict class of the text.
'''
...
column_names = ['log_level', 'static_text']
...
predicted, expected = predict_expected(test_dataloader)

for i in range(len(df_test)):
    text = df_test.iloc[i,1]
    print( f"{text}: Expected {expected[i]} Predicted: {predicted[i]} ")

precision = metrics.accuracy_score(expected, predicted)

recall = metrics.recall_score(expected, predicted, average='macro')
f1 = metrics.f1_score(expected, predicted, average='macro')
...
cmd_obj = ConfusionMatrixDisplay(cm, display_labels=['0', '1', '2'])
cmd_obj.plot()
